defmodule RemoteIpRewriterTest do
  ...
  test "never trusts local proxies" do
    conn = conn(:get, "/") 
           |> set_remote_ip({45, 1, 1, 4}) 
           |> put_xff_header("5.6.7.8, 192.168.1.100") 
           |> remote_ip(trust_local_proxies: false, trusted_proxies: [{45, 1, 1, 4}])
    assert conn.remote_ip == {192, 168, 1, 100}
  end

  test "trusts only proxies defined in trusted_proxies" do
    conn = conn(:get, "/") 
           |> set_remote_ip({45, 1, 1, 4}) 
           |> put_xff_header("1.2.3.4, 5.6.7.8") 
           |> remote_ip(trust_local_proxies: false, trusted_proxies: [{45, 1, 1, 4}, {5, 6, 7, 8}])
    assert conn.remote_ip == {1, 2, 3, 4}
  end
